...
text:
`Tesla has driven a long, potholed road to success. Since its 2010 IPO, the famous (and occasionally infamous) auto
manufacturer has enjoyed stratospheric highs, cratered to purgatorial lows and ails about his discussions to take Tesla
private using Saudi Arabia’s sovereign wealth fund, among other potential backers.s team covered Tesla Autopilot,
training data generation, Project Dojo and the D1 chip, and the Teslat misrepresentation Stock Price: $297.28 .`,
url:
...
text:`"The Federal Reserve’s aggressive rate hikes so far in 2022 have ended the era of free money and exposed a worrisome
dynamic throughout capital markets: zombie stocks. These are companies with poor business models that are burning cash
at an alarming rate and are at risk of seeing their stock decline to $0 per share. While Tesla (TSLA) is not a zombie
stock thanks to Elon Musk"`,
url:
...
text:
`People will file fewer objections against their home valuations this year, the homeowners’ association VEH and the
property tax supervisor Valuation Chamber expect. Last year, municipalities handled around 675,000 objections when WOZ
values rose 17 percent. This year, they’re only increasing by 2.8 percent, NOS reports. WOZ is the Dutch acronym used
for the Real Estate Valuation Act. Every year, municipalities adjust the property values based on home prices in January
the year before. In 2023, the WOZ value rose by a massive 17 percent, on average, because home prices skyrocketed in
2022. The WOZ value is used to calculate various taxes, including property tax. This year, the average WOZ increase is
much lower at 2.8 percent. “Then you experience such a WOZ different than with a significant increase,” Ruud Kathmann of
the Valuation Chamber told NOS. VEH expects far fewer objections this year for that reason. “Last year, people were very
shocked by that 17 percent. A lower percentage feels better,” Hans Andre de la Porte of the homeowners’ interest group
said. New rules also make it much less lucrative for objection agencies to file an objection on behalf of homeowners.
Until last year, objection agencies received many hundreds of euros in expense reimbursements directly from the
municipality if they won a procedure. Now, those fees have been reduced by 75 percent, and the municipality pays them to
the citizens. Objection agencies must arrange for their clients to pay them. The government introduced the rules because
municipalities were spending more and more money on these agencies. “My impression is that objection agencies,
therefore, advertise significantly less,” Kathmann said. That will also result in fewer objections, he thinks.`,
url:
...
const [channelName, id, image, title, content, formatedDate] = getHeroContent(
singleChannel
);
...
//get hero content
const getHeroContent = (channels) => {
let channelName = channels[0].name;
let id = channels[0].news[0].id;
let image = channels[0].news[0].image;
let title = channels[0].news[0].title;
let content = channels[0].news[0].text;
let publishedDate = channels[0].news[0].publish_date;
let formatedDate = formatDate(publishedDate);
return [channelName, id, image, title, content, formatedDate];
};
//get hero welcome content
const getHeroWelcomeContent = () => {
const image =
...
const content = `I'm very glad you're here. To view the daily news, you need to create your channel using the 'Create
Channel' button. Use the 'Default Channel' button to see sample news.`;
let status = "welcome";
const publishedDate = "";
const channelName = null;
return [image, id, title, content, status, publishedDate, channelName];
};
/*.....................
START PAGE
.....................*/
